<template>
  <NuxtLink :to="`/series/${series.id}`" class="block">
    <div class="bg-gray-700 rounded-lg w-auto shadow-lg p-8 hover:bg-gray-600 transition-all hover:scale-102 duration-400 ">
      <img 
        :src="series.image?.original || '/placeholder.jpg'" 
        alt="Poster" 
        class="mx-auto block w-auto h-80 object-cover rounded-lg"
      />
      <h2 class="text-lg font-bold p-2 text-white mx-auto block mt-2">{{ series.name }}</h2>
      <p class="text-yellow-600 font-arial pl-2">
        Rating: {{ series.rating?.average ? series.rating.average + '/10' : 'No rating' }} 
      </p>
...
const shortSummary = computed(() => {
  if (showFull.value || plainText.value.length <= MAX_TEXT_LENGTH) return props.series.summary
