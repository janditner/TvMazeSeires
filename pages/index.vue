  ...
  <div class="pt-1 pb-1 bg-gray-800">
    <ClientOnly>
      <div v-if="search.hasPerformedSearch && !search.isSearching" class="mt-4">
        ...
        <div v-if="search.selectedSeries.length === 0" class="text-center py-10 text-white">
          No results found for "{{ search.query }}"
        </div>
        <div class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 mx-8">
          <div v-for="s in search.selectedSeries.slice(0, 3)" :key="s.id" class="p-4">
      ...
      <div v-else class="mt-4 ml-4 mr-4">
        <div v-if="carouselError" class="text-red-500 text-center mb-4">{{ carouselError }}</div>
        <div v-if="recentSeriesError" class="text-red-500 text-center mb-4">{{ recentSeriesError }}</div>
        <Carousel
